...
namespace ENGINE::PSX {
    ...
    void handleCDROMInterrupt(void) {
        ...
        auto cdinstance = g_CDInstance.get();
        cdinstance->responselen = 0;
        while (CDROM_HSTS & CDROM_HSTS_RSLRRDY)
            cdinstance->response[cdinstance->responselen++] = CDROM_RESULT;

        switch (irqType) {
            case CDROM_IRQ_DATA_READY:
                cdinstance->irqDataReady();
                break;
            case CDROM_IRQ_COMPLETE:
                cdinstance->irqComplete();
                break;
            case CDROM_IRQ_ACKNOWLEDGE:
                cdinstance->irqAcknowledge();
                break;
            case CDROM_IRQ_DATA_END:
                cdinstance->irqDataEnd();
                break;
