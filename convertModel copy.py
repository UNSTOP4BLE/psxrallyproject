...
for l in inlines:
    ...
    f = Face()
    f.indices[:] = indices
    if material is not None and dict(materials).get(material) is not None:
        ...
        f.u[:] = [int((1.0 - uvs[i][0])*width) for i in uvindices] 
        ...
        if material is not None:
            ...
            f.color = 0x808080 #default color, gray
        f.texid = -1
    faces.append(f)
